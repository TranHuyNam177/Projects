...
quantities = [
    'revenue', 'cogs', 'gross_profit', 'interest', 'pbt', 'net_income', 'cur_asset', 'cash',
    'ar', 'lt_receivable', 'inv', 'ppe', 'asset', 'liability', 'cur_liability', 'lt_debt',
    'equity', 'retn_earnings', 'period_retn_earnings', 'lt_invst', 'depre_tgbl', 'depre_gw',
...
df['depi'] = (df['depre_tgbl'] + df['depre_gw']) / (df['ppe'] + df['depre_tgbl'] + df['depre_gw'])
...
df['ebitda/rev'] = (df['pbt'] + df['interest'] + df['depre_tgbl'] + df['depre_gw']) / df['revenue']
...
df['roic'] = df['period_retn_earnings'] / (df['lt_debt'] + df['st_debt'] + df['equity'])
    ...
    for year, quarter in zip(years, quarters):
        ...
        if df_xs.shape[0] < min_tickers:
            ...
            for quantity in quantities_new:
                # remove outliers (Inter quartile Range Method)
    ...
    for col in range(centers.shape[1]):
        if centers.iloc[row, col] is None:
            ...
            for center in range(centroids):
                ...
                distance[center] = ((np.array(centers.iloc[row, col][center]) - np.array(benmrks.iloc[row, col])) ** 2).sum() ** (1 / 2)
    ...
    for col in range(centers.shape[1]):
        if radius_centers.iloc[row, col] is None:
            ...
            for n in range(1, centroids + 1):
                center_scores.iloc[row, col] = np.where(center_scores.iloc[row, col] == n, 100 / (centroids + 1) * n, center_scores.iloc[row, col])
    ...
    for col in range(labels.shape[1]):
        if labels.iloc[row, col] is None:
            ...
            for ticker in range(len(labels.iloc[row, col])):
                distance[ticker] = (((np.array(kmeans_coord.iloc[row, col][ticker])) - np.array(benmrks.iloc[row, col])) ** 2).sum() ** (1 / 2)
...
component_filename = 'component_table_gen'
...
def compare_industry(tickers: list):
    ...
    for ticker in tickers:
        ...
        for row in range(rowsrequired):
            for col in range(chartsperrow):
                ...
                if variables[row, col] is None:
                    ...
                    ax[row, col].set_title(components[variables[row, col]], fontsize=16)
        ...
        fig.legend(
            ...
            bbox_to_anchor=(0.01, 0.98), ncol=2, fontsize=16,
            markerscale=0.7
        )
