...
from function import priceKformat, adjprice
from request.stock import ta

check = '2023q3'
# destination_dir = join(dirname(realpath(__file__)), 'result_chart')
destination_dir = join(dirname(realpath(__file__)), 'result_chart', check)
...
# CREDIT RATING
rating_path = join(realpath(dirname(dirname(__file__))), 'credit_rating', f'result - {check}')
print(rating_path)
...
def run(
    ...
    df.loc[df['close'] == 0, 'close'] = df.loc[df['close'] == 0, 'ref']
    df.loc[df['ref'] == 0, 'ref'] = df.loc[df['ref'] == 0, 'close']
    df['change_percent'] = df['close'] / df['ref'] - 1
    df['logr'] = np.log(1 + df['change_percent'])
    df['change_logr'] = 0
    for i in range(1, len(df.index)):
        df.loc[df.index[i], 'change_logr'] = df.loc[df.index[i], 'logr'] - df.loc[df.index[i - 1], 'logr']
    df.loc[df.index[0], 'change_logr'] = df.loc[df.index[1], 'change_logr']
    df['close'] = df['close'] * 1000
    df.drop(columns=['ref', 'high', 'low', 'change', 'change_percent', 'total_volume', 'total_value'], inplace=True)

    # D'Agostino-Pearson test for log return model (fail the test -> abnormal price movement, manipulated price)
    stat_logr, p_logr = stats.normaltest(df['logr'], nan_policy='omit')
    print(f'p_logr of {ticker} is', np.round(p_logr, 3))

    if p_logr <= alpha:

        # Testing whether normal skewness
        stat_skew, p_skew = stats.skewtest(df['logr'], nan_policy='omit')
        print(f'p_skew of {ticker} is', np.round(p_skew, 3))

        # Testing whether normal kurtosis.
        stat_kur, p_kur = stats.kurtosistest(df['logr'], nan_policy='omit')
        print(f'p_kur of {ticker} is', np.round(p_kur, 3))

        if p_skew <= 0.05 and p_kur <= 0.05:
            loc = np.nanmean(df['logr'])
            scale = np.nanstd(df['logr'])
            np.random.seed(seed)
            logr = np.random.normal(loc, scale, size=(simulation, pdays))
        elif p_skew <= 0.05 < p_kur:
            mean = np.nanmean(df['logr'])
            std = np.nanstd(df['logr'])
            deg_free = df['logr'].count() - 1
            loc = mean
            scale = (std ** 2 * (deg_free - 2) / deg_free) ** 0.5
            logr = stats.t.rvs(deg_free, loc, scale, size=(simulation, pdays), random_state=seed)
        elif p_skew > 0.05 >= p_kur:
            mean = np.nanmean(df['logr'])
            std = np.nanstd(df['logr'])
            skew = stats.skew(df['logr'], nan_policy='omit')
            theta = (np.pi / 2 * abs(skew) ** (2 / 3) / (
                        abs(skew) ** (2 / 3) + ((4 - np.pi) / 2) ** (2 / 3))) ** 0.5 * np.sign(skew)
            a = theta / (1 - theta ** 2) ** 0.5
            scale = (std ** 2 / (1 - 2 * theta ** 2 / np.pi)) ** 0.5
            loc = mean - scale * theta * (2 / np.pi) ** 0.5
            logr = stats.skewnorm.rvs(a, loc, scale, size=(simulation, pdays), random_state=seed)
        else:
            mean = np.nanmean(df['logr'])
            std = np.nanstd(df['logr'])
            deg_free = df['logr'].count() - 1
            ...
            logr = stats.nct.rvs(deg_free, nc, loc, scale, size=(simulation, pdays), random_state=seed)

        # Convert logr back to simulated price
        price_t = df.loc[df['trading_date'] == df['trading_date'].max(), 'close'].squeeze()
        ...
        # Testing whether normal skewness
        stat_skew, p_skew = stats.skewtest(df['change_logr'], nan_policy='omit')
        print(f'p_skew of {ticker} is', np.round(p_skew, 3))
        # Testing whether normal kurtosis
        stat_kur, p_kur = stats.kurtosistest(df['change_logr'], nan_policy='omit')
        print(f'p_kur of {ticker} is', np.round(p_kur, 3))
        ...
        if p_skew <= 0.05 and p_kur <= 0.05:
            ...
            loc = mean
            scale = (std ** 2 * (deg_free - 2) / deg_free) ** 0.5
            change_logr = stats.t.rvs(deg_free, loc, scale,
                                      size=(simulation, pdays),
                                      random_state=seed)
        elif p_skew > 0.05 >= p_kur:
            mean = 0
            std = np.nanstd(df['change_logr'])
            skew = stats.skew(df['change_logr'], nan_policy='omit')
            theta = (np.pi / 2 * skew ** (2 / 3) / (skew ** (2 / 3) + ((4 - np.pi) / 2) ** (2 / 3))) ** 0.5 * np.sign(
                skew)
            a = theta / (1 - theta ** 2) ** 0.5
            scale = (std ** 2 / (1 - 2 * theta ** 2 / np.pi)) ** 0.5
            loc = mean - scale * theta * (2 / np.pi) ** 0.5
            change_logr = stats.skewnorm.rvs(a, loc, scale,
                                             size=(simulation, pdays),
                                             random_state=seed)
        else:
            mean = 0
            ...
            scale = std
            nc = mean * (1 - 3 / (4 * deg_free - 1))
            change_logr = stats.nct.rvs(deg_free, nc, loc, scale, size=(simulation, pdays), random_state=seed)

        # Convert change_logr back to simulated price
        price_t = df.loc[df['trading_date'] == df['trading_date'].max(), 'close'].iloc[0]
    ...
    lv0_price = convert_to_valid_price(lv0_price)
    lv1_price = convert_to_valid_price(lv1_price)
    lv2_price = convert_to_valid_price(lv2_price)
    lv3_price = convert_to_valid_price(lv3_price)
    ...
    if rating in ['Not enough data', 'Not in industry classification']:
        ...
        breakeven_price = lv0_price  # not enough fundamental data for credit rating means convertively making no markup
    elif rating >= 75:
        group = 'A'
        breakeven_price = lv3_price
    elif rating >= 50:
        group = 'B'
        breakeven_price = lv2_price
    elif rating >= 25:
        group = 'C'
        breakeven_price = lv1_price
    else:
        group = 'D'
        breakeven_price = lv0_price
