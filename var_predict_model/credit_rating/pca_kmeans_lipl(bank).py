...
quantities = [
    'balnce_sbv', 'balnce_inst', 'loans_customer', 'trading_sec',
...
for year, quarter in zip(years, quarters):
    ...
    # Kmeans algorithm
    kmeans.loc['bank', f'{year}q{quarter}'] = KMeans(
...
for col in range(centers.shape[1]):
    if centers.iloc[0, col] is None or type(centers.iloc[0, col]) != list:
...
for col in range(labels.shape[1]):
    if labels.iloc[0, col] is None or type(labels.iloc[0, col]) != list:
...
for col in range(radius_tickers.shape[1]):
    if radius_tickers.iloc[0, col] is None or type(radius_tickers.iloc[0, col]) not in (list, np.ndarray):
